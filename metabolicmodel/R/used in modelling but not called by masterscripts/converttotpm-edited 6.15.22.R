...
setwd("C:/Users/msportiello/Box/TRM_Metabolism_Modeling/metabinformatics/results/cd49asp")
expressiondata<-data.frame(CD49a$gene,CD49a$symbol,CD49a$rowmean)
head(expressiondata)
expressiondata<-expressiondata[!is.na(expressiondata$CD49a.gene), ]
names(expressiondata)[1] <- "gene"
names(expressiondata)[2] <- "symbol"
names(expressiondata)[3] <- "tpm"
head(expressiondata)
dim(expressiondata)
write.table(expressiondata,'CD49a.genesymboltpm.txt',row.names = F,col.names =T,quote = F)
#these .txt files were then saved as excels for use in the matlab scripts.
#sessionInfo()
#citation()
...
#gene
#generow<-gene
#write.table(generow,'generowCD49a.txt')
#gene<-as.vector(gene)
#class(gene)
#head(gene)
#writeChar(gene,'genecharCD49a.txt')
...
#head(binary)
#length(binary)
#write.table(gene,'genetableCD49a.txt',row.names=F,quote=F,col.names=F)
#write.table(value,'valuetableCD49a.txt',row.names=F,quote=F,col.names=F)
#write.table(binary,'binarytableCD49a.txt',row.names=F,quote=F,col.names=F)

########################

setwd("C:/Users/msportiello/Box/TRM_Metabolism_Modeling/metabinformatics/results/cd103sp")
expressiondata<-data.frame(CD103$gene,CD103$symbol,CD103$rowmean)
head(expressiondata)
expressiondata<-expressiondata[!is.na(expressiondata$CD103.gene), ]
names(expressiondata)[1] <- "gene"
names(expressiondata)[2] <- "symbol"
names(expressiondata)[3] <- "tpm"
head(expressiondata)
dim(expressiondata)
write.table(expressiondata,'CD103.genesymboltpm.txt',row.names = F,col.names =T,quote = F)
#these .txt files were then saved as excels for use in the matlab scripts.
sessionInfo()
citation()
...
#gene
#generow<-gene
#write.table(generow,'generowCD103.txt')
#gene<-as.vector(gene)
#class(gene)
#head(gene)
#writeChar(gene,'genecharCD103.txt')
...
#head(binary)
#length(binary)
#write.table(gene,'genetableCD103.txt',row.names=F,quote=F,col.names=F)
#write.table(value,'valuetableCD103.txt',row.names=F,quote=F,col.names=F)
#write.table(binary,'binarytableCD103.txt',row.names=F,quote=F,col.names=F)
